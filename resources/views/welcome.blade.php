...
<body
    class="premium-bg min-h-screen relative overflow-x-hidden selection:bg-amber-100 selection:text-amber-900 flex flex-col">

    <!-- Background Accents -->
    <div class="mesh-decoration top-0 left-0 w-[500px] h-[500px] -translate-x-1/3 -translate-y-1/3"></div>
    <div class="mesh-decoration bottom-0 right-0 w-[600px] h-[600px] translate-x-1/3 translate-y-1/3 !bg-blue-500">
    </div>
        ...
        <div class="max-w-7xl mx-auto">
            <div class="glass-panel rounded-2xl px-6 py-3 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3 md:gap-4">
                    <div
                        class="w-10 h-10 md:w-11 md:h-11 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/20 text-white">
                ...
                <div class="flex items-center gap-2 md:gap-3">
                    <a href="{{ route('calendar.pdf', ['date' => $selectedDate->format('Y-m-d')]) }}" target="_blank"
    ...
    <!-- Main Content -->
    <main class="pt-32 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-12">
        ...
        <section class="grid lg:grid-cols-12 gap-8 items-start">
            ...
            <div
                class="lg:col-span-5 flex flex-col items-center lg:items-start text-center lg:text-left space-y-6 reveal-up">
                <!-- Date Badge -->
                <div
                    class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-50 border border-amber-100 text-amber-700 text-sm font-medium animate-float">
                ...
                <div>
                    ...
                    <div class="flex items-baseline gap-2 lg:block">
                        <h1 class="text-8xl md:text-9xl font-bold text-transparent bg-clip-text bg-gradient-to-br from-slate-900 via-slate-700 to-slate-900 leading-none tracking-tighter"
                            style="font-family: 'Outfit', sans-serif;">
                            {{ $thaiDay }}
                        </h1>
                        <p class="text-3xl md:text-4xl text-amber-600 font-medium lg:-mt-2">{{ $thaiMonth }}
                            {{ $thaiYear }}</p>
                    </div>
                </div>
                ...
                <!-- Realtime Clock Card -->
                <div
                    class="w-full max-w-xs glass-card rounded-2xl p-5 border-l-4 border-amber-500 flex items-center justify-between">
                    <div>
            ...
            <div class="lg:col-span-7 space-y-6 reveal-up delay-100">
                ...
                <div class="glass-panel rounded-3xl p-6 md:p-8 relative overflow-hidden">
                    <div class="absolute top-0 right-0 w-32 h-32 bg-blue-500/5 rounded-full blur-2xl -mr-10 -mt-10">
                    </div>
                    ...
                    <form id="filterForm" method="GET" action="{{ route('home') }}" class="relative z-10 space-y-6">
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-slate-100">
                            <div class="flex gap-3">
                                <a href="{{ route('home') }}"
                                    class="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm font-medium transition-colors">
                                    <i class="fa-solid fa-calendar-day mr-1"></i> วันนี้
                                </a>
                                @if(request('date') || request('staff_id'))
                                    <a href="{{ route('home') }}"
                                        class="px-4 py-2 rounded-lg bg-red-50 hover:bg-red-100 text-red-600 text-sm font-medium transition-colors">
                ...
                <div
                    class="glass-panel rounded-2xl p-4 flex flex-wrap gap-3 items-center justify-center md:justify-start">
                    @foreach($allStaff as $person)
                        ...
                        <a href="{{ route('home', ['date' => $selectedDate->format('Y-m-d'), 'staff_id' => $person->id]) }}"
                            ...
                            title="{{ $person->name }}">
                            <div
                                class="w-12 h-12 rounded-full border-2 {{ $selectedStaffId == $person->id ? 'border-amber-500 shadow-lg shadow-amber-500/20' : 'border-white' }} overflow-hidden">
                                <img src="{{ $person->photo_url ?? asset('images/default-avatar.png') }}"
                                    class="w-full h-full object-cover">
                            </div>
                            <!-- Status Dot -->
                            @if($hasEvent)
                                <div
                                    class="absolute bottom-0 right-0 w-3.5 h-3.5 bg-amber-500 border-2 border-white rounded-full">
                                </div>
                            @endif
                        </a>
                    @endforeach
        ...
        <section class="reveal-up delay-200">
            <div
                class="relative rounded-3xl overflow-hidden bg-gradient-to-r from-slate-900 to-slate-800 shadow-2xl text-white">
                <!-- Decorative Blobs -->
                <div
                    class="absolute top-0 right-0 w-64 h-64 bg-amber-500 rounded-full mix-blend-overlay filter blur-3xl opacity-20 -mr-16 -mt-16 animate-pulse">
                </div>
                <div
                    class="absolute bottom-0 left-0 w-64 h-64 bg-blue-500 rounded-full mix-blend-overlay filter blur-3xl opacity-20 -ml-16 -mb-16">
                </div>
                ...
                <div class="relative p-6 md:p-10 flex flex-col md:flex-row items-center gap-8">
                    ...
                    <div class="flex-shrink-0">
                        <div
                            class="w-16 h-16 rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center border border-white/10 shadow-inner">
                    ...
                    <div class="flex-1 w-full overflow-hidden" id="slider-container">
                        ...
                        <div class="relative h-24 md:h-16">
                            @foreach($announcements as $index => $ann)
                                <div class="announcement-slide absolute top-0 left-0 w-full transition-all duration-700 ease-in-out {{ $index === 0 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8' }}"
                                    data-index="{{ $index }}">
                                    <h3
                                        class="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-300 mb-1">
                                        {{ $ann['title'] }}
                                    </h3>
                    ...
                    <div class="flex gap-2">
                        @foreach($announcements as $index => $ann)
                            <button onclick="switchSlide({{ $index }})"
                                class="dot-indicator w-2 h-2 rounded-full transition-all {{ $index === 0 ? 'bg-amber-400 w-6' : 'bg-white/20 hover:bg-white/40' }}"
        ...
        <section class="max-w-none">
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-6 lg:gap-8">
                @forelse($staffList as $staff)
                    <div class="glass-card rounded-3xl p-0 overflow-hidden group reveal-up"
                        ...
                        <div class="bg-gradient-to-r from-slate-50 to-white border-b border-slate-100 p-6 md:p-8 relative">
                            <!-- Background Pattern -->
                            <div
                                class="absolute top-0 right-0 p-6 opacity-5 md:opacity-10 transition-opacity group-hover:opacity-20 pointer-events-none">
                            ...
                            <div class="relative flex items-center gap-6 z-10">
                                ...
                                <div class="relative flex-shrink-0">
                                    <div
                                        class="w-24 h-24 rounded-2xl overflow-hidden shadow-2xl border-4 border-white group-hover:border-amber-400 transition-colors duration-300">
                                        @if($staff->photo)
                                    ...
                                    <div
                                        class="absolute -bottom-3 -right-3 w-10 h-10 rounded-xl bg-slate-900 text-white flex items-center justify-center font-bold text-sm shadow-lg border-2 border-white">
                                ...
                                <div>
                                    <h3
                                        class="text-2xl font-bold text-slate-800 group-hover:text-amber-600 transition-colors">
                                        {{ $staff->name }}</h3>
                                    <p class="text-amber-600 font-medium text-sm md:text-base mt-0.5">{{ $staff->position }}
                                    </p>
                                    @if($staff->department)
                                        <div
                                            class="inline-flex items-center gap-1.5 mt-2 px-2.5 py-1 rounded-md bg-slate-100 text-slate-500 text-xs font-medium">
                                ...
                                <div class="relative space-y-0">
                                    ...
                                    @foreach($staff->calendarEvents as $event)
                                        <div class="relative pl-10 pb-6 last:pb-0 group/event">
                                            <!-- Timestamp Bullet -->
                                            <div
                                                class="absolute left-0 top-1.5 w-7 h-7 rounded-full bg-white border-2 border-amber-500 flex items-center justify-center z-10 shadow-sm group-hover/event:scale-110 transition-transform">
                                                <div class="w-2.5 h-2.5 rounded-full bg-amber-500"></div>
                                            </div>

                                            <div
                                                class="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 hover:border-amber-200 hover:shadow-md transition-all duration-300 group-hover/event:translate-x-1">
                                                <div class="flex flex-wrap items-center justify-between gap-y-1 mb-2">
                                                    <span
                                                        class="inline-flex items-center gap-1.5 text-sm font-bold text-amber-600 bg-amber-50 px-2.5 py-1 rounded-lg">
                                                        <i class="fa-regular fa-clock text-xs"></i>
                                                        {{ \Carbon\Carbon::parse($event->start_time)->format('H:i') }}
                                                        @if($event->end_time) -
                                                        {{ \Carbon\Carbon::parse($event->end_time)->format('H:i') }} @endif
                                                    </span>
                                                    @if($event->location)
                                                        <span
                                                            class="text-xs text-slate-500 flex items-center gap-1 max-w-[150px] truncate">
                                                ...
                                                @if($event->note)
                                                    <div class="mt-3 pt-3 border-t border-slate-100">
                                                        <p
                                                            class="text-sm text-amber-700 bg-amber-50 px-3 py-2 rounded-lg flex items-start gap-2">
                                                            <i class="fa-solid fa-note-sticky mt-1 text-amber-500 text-xs"></i>
                                                            <span class="font-medium">หมายเหตุ: {{ $event->note }}</span>
                                                        </p>
                                                    </div>
                                                @endif
                                            </div>
            ...
            <div class="flex flex-col md:flex-row items-center justify-between gap-6">
                <div class="flex items-center gap-4">
                    <img src="{{ asset('images/logo.png') }}"
                        class="w-12 h-12 object-contain grayscale opacity-50 hover:grayscale-0 hover:opacity-100 transition-all duration-500"
                        alt="Logo">
                ...
                <div class="flex items-center gap-6">
                    ...
                    <a href="#"
                        class="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 hover:bg-slate-200 hover:text-slate-600 transition-colors">
